...
import { deleteEntity, getEntities, reset } from './meeting.reducer';
...
export const Meeting = () => {
  ...
  const links = useAppSelector(state => state.meeting.links);
  const updateSuccess = useAppSelector(state => state.meeting.updateSuccess);
  const [loadModal, setLoadModal] = useState(false);
  const [meetingToDeleteId, setMeetingToDeleteId] = useState(null);
  const handleClose = () => {
    setLoadModal(false);
  };
  const openModal = id => {
  ...
  const confirmDelete = () => {
    if (meetingToDeleteId) {
      dispatch(deleteEntity(meetingToDeleteId));
  ...
  return (
    <div>
      ...
      <div className="table-responsive">
        <InfiniteScroll
          dataLength={meetingList ? meetingList.length : 0}
          next={handleLoadMore}
        ...
        >
          {meetingList && meetingList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr className="text-gray-500">
                  ...
                  <th className="hand" onClick={sort('meetingLink')}>
                    Meeting Link <FontAwesomeIcon icon={getSortIconByFieldName('meetingLink')} />
                  </th>
                  <th>
              ...
              <tbody>
                {meetingList.map((meeting, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td className="py-4">
                      {meeting.teacher ? <Link to={`/teacher/${meeting.teacher.id}`}>{meeting.teacher.name}</Link> : ''}
                    </td>
                    <td className="py-4">
                      {meeting.timeslot ? <Link to={`/timeslot/${meeting.timeslot.id}`}>{meeting.timeslot.startTime}</Link> : ''}
                    </td>
                    <td className="py-4">
                      {meeting.student ? <Link to={`/student/${meeting.student.id}`}>{meeting.student.firstName}</Link> : ''}
                    </td>
            ...
            !loading && <div className="alert alert-warning my-4">No Meetings found</div>
          )}
        </InfiniteScroll>
        <Modal isOpen={loadModal} toggle={handleClose}>
          ...
          <ModalBody id="educationclickApp.meeting.delete.question">Are you sure you want to delete meeting?</ModalBody>
          <ModalFooter>
