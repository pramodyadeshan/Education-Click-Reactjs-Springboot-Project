...
import { deleteEntity, getEntities, reset } from './booking.reducer';
...
export const Booking = () => {
  ...
  const confirmDelete = () => {
    if (bookingToDeleteId) {
      dispatch(deleteEntity(bookingToDeleteId));
      ...
      <div className="table-responsive">
        ...
        >
          {bookingList && bookingList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr className="text-gray-500">
                  ...
                  <th>
                    Student Name <FontAwesomeIcon icon="sort" />
                  </th>
              ...
              <tbody>
                {bookingList.map((booking, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td className="py-4">
                      {booking.bookingDate ? <TextFormat type="date" value={booking.bookingDate} format={APP_DATE_FORMAT || '-'} /> : null}
                    </td>
                    <td className="py-4">
                      {booking.timeslot ? <Link to={`/timeslot/${booking.timeslot.id}`}>{booking.timeslot.startTime || '-'}</Link> : ''}
                    </td>
                    <td className="py-4">
                      {booking.teacher ? <Link to={`/teacher/${booking.teacher.id}`}>{booking.teacher.name || '-'}</Link> : ''}
                    </td>
                    <td className="py-4">
                      {booking.student ? <Link to={`/student/${booking.student.id}`}>{booking.student.firstName || '-'}</Link> : ''}
                    </td>
                    <td className="py-4 text-bold">LKR {parseInt(booking.payment).toFixed(2) || '0.00'}</td>
            ...
            !loading && <div className="alert alert-warning my-4">No Bookings found</div>
          )}
        </InfiniteScroll>
        <Modal isOpen={loadModal} toggle={handleClose}>
          ...
          <ModalBody id="educationclickApp.booking.delete.question">Are you sure you want to delete booking?</ModalBody>
          <ModalFooter>
